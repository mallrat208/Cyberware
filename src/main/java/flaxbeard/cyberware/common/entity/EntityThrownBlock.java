...
import flaxbeard.cyberware.common.block.item.ItemSurgeryTable;
...
public class EntityThrownBlock extends Entity implements IProjectile
{
			...
			else
			{
				...
				if (!this.world.isRemote)
				{
					...
					if (this.onGround)
					{
						...
						if (iblockstate.getBlock() != Blocks.PISTON_EXTENSION)
						{
							...
							{
								//if (this.world.canBlockBePlaced(block, blockpos1, true, EnumFacing.UP, (Entity)null, ItemStack.EMPTY) && !BlockFalling.canFallThrough(this.world.getBlockState(blockpos1.down())) && this.world.setBlockState(blockpos1, fallTile, 3))
								if (this.world.mayPlace(block, blockpos1, true, EnumFacing.UP, null) && !BlockFalling.canFallThrough(this.world.getBlockState(blockpos1.down())) && this.world.setBlockState(blockpos1, fallTile, 3))
								{
									List<Entity> list = Lists.newArrayList(this.world.getEntitiesWithinAABBExcludingEntity(this, this.getEntityBoundingBox().expand(.5, .5, .5)));
								...
								{
									int num = block.quantityDropped(fallTile, 0, world.rand);
									Item item = block.getItemDropped(fallTile, world.rand, 0);
				...
				if (fallTime > 2)
				{
					...
					if (count > 0 && (ticksExisted - lastAttackTime) > 10)
					{
						...
						world.playSound(posX, posY, posZ, block.getSoundType().getBreakSound(), SoundCategory.AMBIENT, 1, 1, false);
			...
			float velocity, float inaccuracy)
	{
		...
		this.motionX = vec.x * velocity;
		this.motionY = vec.y * velocity;
		this.motionZ = vec.z * velocity;
