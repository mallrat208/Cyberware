...
public class ItemCyberarmTool extends ItemCyberware implements ISidedLimb, ILimbReplacement
{
	...
	private static final ResourceLocation textureChainsaw=new ResourceLocation(Cyberware.MODID, "textures/models/player_chainsaw_arm.png");
	private static final ResourceLocation textureDrill=new ResourceLocation(Cyberware.MODID, "textures/models/player_drill_arm.png");
	private static final ResourceLocation textureLifter=new ResourceLocation(Cyberware.MODID, "textures/models/player_lift_arm.png");
	...
	public Object getModel(ItemStack itemStack, boolean wideArms, Object baseWide, Object baseSkinny, EntityPlayer player)
	{
		...
		if((itemStack.getItemDamage()/2)==0)
		{
			...
			{
				float t=Minecraft.getMinecraft().player.ticksExisted+Minecraft.getMinecraft().getRenderPartialTicks();
				if(Minecraft.getMinecraft().player.getSwingProgress(Minecraft.getMinecraft().getRenderPartialTicks())>0&&itemStack.equals(getActive(Minecraft.getMinecraft().player)))
				{
		...
		else
		{
			...
			if(player == Minecraft.getMinecraft().player)
			{
				boolean isLeft=((ICyberware)CyberwareAPI.getCyberware(itemStack)).getFirstSlot(itemStack)==EnumSlot.ARM_LEFT;
				float t=Minecraft.getMinecraft().player.ticksExisted+Minecraft.getMinecraft().getRenderPartialTicks();
				...
				if(tag.hasKey("blockData"))
				{
					...
					ItemStack stack=b.getItem(player.world, new BlockPos(0, 0, 0), b.getStateFromMeta(meta));
				...
				else
				{
					...
					if(isLeft)
					{
						modelLifter.stackLeft=ItemStack.EMPTY;
					}
					else
					{
						modelLifter.stackRight=ItemStack.EMPTY;
					}
				}
	...
	public static boolean isActive(EntityPlayer p)
	{
		...
		if(!leftLimb.isEmpty()&&leftLimb.getItem() instanceof ItemCyberarmTool)
		{
			isLeftInstalled=true;
		}
		if(!rightLimb.isEmpty()&&rightLimb.getItem() instanceof ItemCyberarmTool)
		{
	...
	public static ItemStack getActive(EntityPlayer p)
	{
		...
		if(p.getPrimaryHand()==EnumHandSide.LEFT)
		{
			if(isRightInstalled&&((ItemCyberarmTool)rightLimb.getItem()).showAsOffHandIfMainhandEmpty(rightLimb))
				return rightLimb;
			
			return leftLimb;
		} else
		{
			if(isLeftInstalled&&((ItemCyberarmTool)leftLimb.getItem()).showAsOffHandIfMainhandEmpty(leftLimb))
				return leftLimb;
	...
	public void handleBreakSpeed(PlayerEvent.BreakSpeed event)
	{
		if (!proc)
		{
			...
			if (!active.isEmpty())
			{
				...
				p.inventory.mainInventory.set(p.inventory.currentItem,new ItemStack(Items.APPLE));
				float bspeed = p.getDigSpeed(event.getState(), event.getPos());
				ItemStack replace = getMiningItem(active);
				p.inventory.mainInventory.set(p.inventory.currentItem, replace);
				float speed = p.getDigSpeed(event.getState(), event.getPos());
				p.inventory.mainInventory.set(p.inventory.currentItem, store);
	...
	public void tick(ClientTickEvent event)
	{
		...
		if(event.phase == Phase.START && CyberwareAPI.hasCapability(mc.player))
		{
			if (mc.gameSettings.keyBindSwapHands.isKeyDown() && mc.currentScreen == null)
			{
				...
				{
					wasPressed = true;
					ItemStack leftLimb = CyberwareAPI.getCapability(mc.player).getLimb(EnumSlot.ARM, EnumSide.LEFT);
					ItemStack rightLimb = CyberwareAPI.getCapability(mc.player).getLimb(EnumSlot.ARM, EnumSide.RIGHT);
					boolean wasActive = isActive(mc.player);
					
					if (!leftLimb.isEmpty() && leftLimb.getItem() instanceof ItemCyberarmTool)
					{
						
						boolean state = getEnabledState(leftLimb);
						...
						toggleEnabledState(leftLimb, !state);
						CyberwarePacketHandler.INSTANCE.sendToServer(new ToggleCyberarmPacket(true, !state));
					}
					if (!rightLimb.isEmpty() && rightLimb.getItem() instanceof ItemCyberarmTool)
					{
						boolean state = getEnabledState(rightLimb);
						toggleEnabledState(rightLimb, !state);
						CyberwarePacketHandler.INSTANCE.sendToServer(new ToggleCyberarmPacket(false, !state));
					}
					
					if (!isActive(mc.player) && wasActive && !mc.player.getHeldItemMainhand().isEmpty())
					{
						mc.player.getAttributeMap().applyAttributeModifiers(mc.player.getHeldItemMainhand().getAttributeModifiers(EntityEquipmentSlot.MAINHAND));
					}
				}
			}
			else
		...
		if (!event.isCanceled() && !active.isEmpty() && (active.getItemDamage() / 2 == 2))
		{
			...
			if (tag.hasKey("blockData"))
			{
				if (!w.isRemote)
				{
					...
					falling.motionX = p.getLookVec().x * 1;
					falling.motionY = p.getLookVec().y * 1 + 0.5F;
					falling.motionZ = p.getLookVec().z * 1;
					falling.fallTime = 1;
					...
					if (w.getBlockState(pos).getBlock().isReplaceable(w, pos))
					{
						...
						if (b.canPlaceBlockOnSide(w, pos, event.getFace().getOpposite()))
						{
							if (!w.isRemote)
							{
								...
								{
									NBTTagCompound teTag = data.getCompoundTag("te");
									TileEntity te = TileEntity.create(w, teTag);
									w.setTileEntity(pos, te);
								}
								
								//w.notifyBlockOfStateChange(pos, b);
								w.notifyNeighborsOfStateChange(pos, b, true);
