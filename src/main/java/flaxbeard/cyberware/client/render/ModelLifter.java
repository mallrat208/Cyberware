...
public class ModelLifter extends ModelPlayer implements ISpecialArmRenderer
{
	...
	public void render(Entity entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scale)
	{
		...
		if (!bipedLeftArm.isHidden)
		{
			...
			Minecraft.getMinecraft().getItemRenderer().renderItem((EntityLivingBase) entityIn, stackLeft, TransformType.FIXED);
		...
		if (!bipedRightArm.isHidden)
		{
			...
			GlStateManager.rotate(270F, 1, 0, 0);
			GlStateManager.translate(-0.075F, 0F, 0.75F);
			Minecraft.getMinecraft().getItemRenderer().renderItem((EntityLivingBase) entityIn, stackRight, TransformType.FIXED);
			GlStateManager.popMatrix();
		}
