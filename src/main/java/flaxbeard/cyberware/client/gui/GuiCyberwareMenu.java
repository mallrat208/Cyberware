	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks)
	{
		...
		if (isPointInRegion(sx + 1 * 2, sy, 6 * 2, 8 * 2, mouseX, mouseY))
		{
			drawHoveringText(Arrays.asList(I18n.format("cyberware.gui.keybind.0"),
			                               I18n.format("cyberware.gui.keybind.1"),
			                               I18n.format("cyberware.gui.keybind.2"),
			                               I18n.format("cyberware.gui.keybind.3"),
			                               I18n.format("cyberware.gui.keybind.4")), mouseX, mouseY, fontRenderer);
		}
			...
			/*for (int i = 0; i < mc.gameSettings.keyBindsHotbar.length; i++)
			{
				...
				if (kb.getKeyCode() == keyCode)
				{
					...
					if (HotkeyHelper.getHotkey(cyberwareUserData.getActiveItems().get(selectedPart)) != i)
					{
						...
						HotkeyHelper.assignHotkey(cyberwareUserData, cyberwareUserData.getActiveItems().get(selectedPart), i);
